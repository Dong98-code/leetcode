...
<div class="content__1Y2H"><div class="notranslate"><p>字符串 <code>s</code> 可以按下述步骤划分为若干长度为 <code>k</code> 的组：</p>
...
var divideString = function(s, k, fill) {
    ...
    if (res > 0) {
        for (let i=0;i<k-res;i++) {
            s += fill;
        }
    }
    let ans = [];
    for (let i=0;i<Math.ceil(n/k);i++) {
        ans.push(s.slice(i*k, (i+1)*k));
    }
    return ans; 
};
```
