...
<div class="notranslate"><p>总共有 <code>n</code>&nbsp;个颜色片段排成一列，每个颜色片段要么是&nbsp;<code>'A'</code>&nbsp;要么是&nbsp;<code>'B'</code>&nbsp;。给你一个长度为&nbsp;<code>n</code>&nbsp;的字符串&nbsp;<code>colors</code>&nbsp;，其中&nbsp;<code>colors[i]</code>&nbsp;表示第&nbsp;<code>i</code>&nbsp;个颜色片段的颜色。</p>
...
因为不能从俩边删除， 只能删除 两边都是 相同颜色的中间的哪一个， 要是能操作：
那么起码为3个连续的字符， 所以统计"AAA" 出现的次数即可

技巧： 
用  `cur` 表示 前一个字符的情况， 如果遍历到现在的字符 和之前的相等， 连续 字符值 +1；
...
var winnerOfGame = function(colors) {
    let cnt = [0, 0]
    let cur = "C"  // 当前的字符值
    let tmp = 0;  // 相同的连续字符的个数
    for (let i = 0;i < colors.length;i++) {
        if (colors[i] !== cur) {// cur此时为前一个字符 不相等 
            tmp = 1;  
            cur = colors[i];
        } else {
            tmp += 1
            if (tmp >= 3) {
                cnt[cur.charCodeAt() - "A".charCodeAt()] += 1;
            }
        }
    }
    return cnt[0] > cnt[1];
