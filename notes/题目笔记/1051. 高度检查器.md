...
<div class="notranslate"><p>学校打算为全体学生拍一张年度纪念照。根据要求，学生需要按照 <strong>非递减</strong> 的高度顺序排成一行。</p>
...
注意 `Array.sort（）`会改变原数组， 需要先将原始数组复制
...
var heightChecker = function(heights) {
    const e = new Array(heights.length).fill(0);
    e.splice(0, heights.length, ...heights);
    e.sort((a, b) => a- b)  // sort 方法该边原数组
    let res = 0;
